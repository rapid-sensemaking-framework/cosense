...
function TemplatePreviewElement({ label, children }) {
  return <div className="template-preview-element">
...
export default function TemplatePreview({ template, formData }) {
  ...
  const mainInput = template.expectedInputs.find(e => e.port === CONTACTABLE_CONFIG_PORT_NAME && !e.process.includes('SendMessageToAll'))
  const mainIdent = `${mainInput.process}--${mainInput.port}`
  const mainContactables = formData[mainIdent]
  const sendMessageInput = template.expectedInputs.find(e => e.port === CONTACTABLE_CONFIG_PORT_NAME && e.process.includes('SendMessageToAll'))
  ...
  const nonContactablesInputs = template.expectedInputs.filter(e => e.port !== CONTACTABLE_CONFIG_PORT_NAME)
  ...
  return <div className="template-preview">
    {nonContactablesInputs.map((expectedInput) => {
      ...
      return <TemplatePreviewElement key={ident} label={expectedInput.shortLabel}>
        {input}
      </TemplatePreviewElement>
    })}
    <TemplatePreviewElement label={`Your participants (${mainContactables.length})`}>
